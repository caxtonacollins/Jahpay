...
export function FiatInput({
  ...
  const selectedCurrency =
    FIAT_CURRENCIES.find((f) => f.code === currency) ?? FIAT_CURRENCIES[0];
          ...
          <AnimatePresence>
            {open && (
              <motion.div
                ...
                className="absolute left-0 top-full mt-2 w-52 rounded-xl bg-[#111827] border border-white/[0.1] shadow-2xl shadow-black/60 z-[100] overflow-hidden py-1"
              >
                {FIAT_CURRENCIES.map((f) => (
                  <button
                    key={f.code}
                    onClick={() => {
                      onCurrencyChange(f.code);
                      setOpen(false);
                    }}
